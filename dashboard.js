// DONNÉES
const DATA={
'01':{summary:{totalCA:312960,totalVisites:168,totalProspection:73,totalWilayas:12},
commerciaux:[
...
function updateStar(d){
    ...
    const star=d.commerciaux.reduce((a,b)=>((a.ca/a.objectifCA)>(b.ca/b.objectifCA)?a:b));
    const pct=Math.round(star.ca/star.objectifCA*100);
    document.getElementById('starCommercial').innerHTML=`<div class="star-card"><div class="star-avatar">${star.initiales}</div><div class="star-name">${star.nom}</div><div class="star-stats">${star.wilaya} • ${star.visites} visites</div><div class="star-ca">${fmt(star.ca)}</div><span class="badge ${getStatus(pct)}">${pct}% obj</span></div>`;
}
